...
class ChatCRUD():
    ...
    async def attach_session_to_user_async(
        ...
        try:
            ...
            await db.rollback()
            logger.error(f"Integrity error during attaching session to user: {str(e)}", exc_info=True)
            raise DatabaseError(message="Integrity error during attaching session to user") from e
        except SQLAlchemyError as e:
            await db.rollback()
            logger.error(f"Database error during attaching session to user: {str(e)}", exc_info=True)
            raise DatabaseError(message="Database error during attaching session to user") from e
        except Exception as e:
            await db.rollback()
            logger.error(f"Unexpected error during attaching session to user: {str(e)}", exc_info=True)
            raise DatabaseError(message="Unexpected error during attaching session to user") from e
